#ifndef COGLET_LEXER_H
#define COGLET_LEXER_H
#include "source.h"
#include "utils/string_view.h"

typedef enum {
    TOK_EOF = 0,
    TOK_ERROR,

    // literals
    TOK_NUMBER_INT,     // 123
    TOK_NUMBER_FLOAT,   // 3.14, 1e3, 1.25e-4, 0x1.8p1
    TOK_STRING,         // "hello"
    TOK_CHAR,           // 'a'
    TOK_IDENT,          // foo
    TOK_TRUE,
    TOK_FALSE,
    TOK_NULL,

    // keywords
    TOK_IF, TOK_ELSE, TOK_WHILE, TOK_FOR, TOK_RETURN, TOK_DEFER, TOK_DISCARD, TOK_STATIC_ASSERT,
    TOK_VOID, TOK_STRUCT, TOK_RESOURCE, TOK_UNION, TOK_BREAK, TOK_CONTINUE, TOK_ENUM,
    TOK_SWITCH, TOK_CASE, TOK_DEFAULT, TOK_CAST, TOK_TRUNCATE, TOK_IN,
    TOK_REINTERPRET, TOK_MOVE, TOK_READONLY, TOK_VOLATILE, TOK_OPAQUE, TOK_CFN,
    TOK_SIZE_OF, TOK_ALIGN_OF, TOK_ASM,

    // types
    TOK_S8, TOK_S16, TOK_S32, TOK_S64,
    TOK_U8, TOK_U16, TOK_U32, TOK_U64,
    TOK_ISIZE, TOK_USIZE,
    TOK_F32, TOK_F64,
    TOK_BOOL,
    TOK_INT_KW, TOK_UINT_KW, // pointer-width aliases

    // operators
    TOK_PLUS,
    TOK_MINUS,
    TOK_STAR,
    TOK_SLASH,
    TOK_PERCENT,

    TOK_PLUS_PLUS,
    TOK_MINUS_MINUS,

    TOK_PLUS_EQUAL,
    TOK_MINUS_EQUAL,
    TOK_STAR_EQUAL,
    TOK_SLASH_EQUAL,
    TOK_PERCENT_EQUAL,

    TOK_EQUAL,
    TOK_EQUAL_EQUAL,
    TOK_BANG,
    TOK_BANG_EQUAL,

    TOK_LESS,
    TOK_LESS_EQUAL,
    TOK_GREATER,
    TOK_GREATER_EQUAL,

    TOK_SHIFT_LEFT,
    TOK_SHIFT_RIGHT,
    TOK_SHIFT_LEFT_EQUAL,
    TOK_SHIFT_RIGHT_EQUAL,

    TOK_AND_AND,
    TOK_OR_OR,

    TOK_AND,
    TOK_OR,
    TOK_XOR,
    TOK_TILDE,

    TOK_AND_EQUAL,
    TOK_OR_EQUAL,
    TOK_XOR_EQUAL,

    // punctuation
    TOK_LPAREN,
    TOK_RPAREN,
    TOK_LBRACE,
    TOK_RBRACE,
    TOK_LBRACKET,
    TOK_RBRACKET,
    TOK_SEMICOLON,
    TOK_COMMA,
    TOK_DOT,
    TOK_ELLIPSIS,
    TOK_ARROW,
    TOK_COLON,
    TOK_COLON_COLON,
    TOK_COLON_EQUAL,
    TOK_HASH,

} TokenType;

// Tokens do NOT own their text: `start` points into the original
// source buffer, which must stay alive for the whole compile.
typedef struct {
    TokenType type;

    //StringView start;

    const char *start;
    int length;

    int line;
    int column;

    /* Stable source provenance for AST/semantic/backend diagnostics. */
    SourceSpan span;
} Token;

typedef struct {
    const char *filename;      // for error reporting
    SourceFileId source_id;
    const char *source_start;  // kept for error reporting
    const char *current;
    int line;
    int column;
    const char *line_start;
    const char *error_msg;     // set when type == TOK_ERROR
} Lexer;

void lexer_init(Lexer *lx, const char *filename, const char *source);
void lexer_init_with_source_id(
    Lexer *lx,
    SourceFileId source_id,
    const char *filename,
    const char *source
);
Token lexer_next(Lexer *lx);
const char *token_type_name(TokenType type); // for printing/tests

#endif
