...
- **Resource ownership is move-only but deliberately not borrow-checked.** `resource` prevents ordinary by-value copying and `move` invalidates direct local/parameter owners, but raw pointers and slices remain freely copyable aliases. The compiler does not prove that an alias dies before resource destruction, arena reset, or reallocation. This is intentional for Coglet's explicit systems model, not a missing plan to add Rust-style borrowing.
- **Resource cleanup is still explicit.** There are no automatic destructors/drop glue. Owning values normally pair `defer value.deinit()` with explicit transfer via `move`. A defer that directly references an owner prevents a later move in the same active lexical scope, but aliases hidden behind pointers can still outlive or clean up moved storage if the programmer mismanages them.
- **Moving out of resource fields is not implemented.** The initial `move` operand must be a direct local or parameter. Resource structs may contain resource fields, but transferring an individual field owner requires a future explicit field-move design.
- **Resource globals are not supported yet.** The first ownership dataflow model is function-local. Global owning resources need explicit initialization/teardown ordering before they are admitted.
- **Resource-valued container elements are deliberately rejected.** `Array<T>` and `Pool<T>` require the closed builtin `copyable` constraint. Add element-wise move-in/move-out/destruction semantics before relaxing that boundary for resource-valued `T`.
...
- **Scratch scopes are conventionally LIFO, not compiler-enforced.** `Scratch.deinit()` rewinds its arena to a captured checkpoint. Ending nested scratch scopes out of order, or using allocator handles/pointers/slices after rewind, is programmer error; there is intentionally no borrow/lifetime checker.
- **`FixedArena` borrows its backing byte buffer without lifetime tracking.** Moving or destroying the arena does not own/free that buffer, and using an allocator from the fixed arena after the buffer goes out of scope or after `reset()` invalidates expected-live allocations. Runtime bookkeeping consumes a small aligned prefix of the buffer.
- **Fixed-arena resize is allocate/copy, not top-of-stack in-place growth.** Repeated resizing can consume additional fixed-buffer space until `reset()`. A later top-allocation optimization can improve this without changing the allocator ABI.
- **`Pool<T>` is fixed-capacity and handle-oriented.** Generation checks reject normal stale-handle reuse, but generations are finite `u64` counters and would eventually wrap after an impractically large number of reuse cycles. The first pool has no growth, iteration protocol, compaction, or resource-element ownership semantics.
- **Public aggregate fields limit container encapsulation.** `Pool<T>` exposes its representation fields because Coglet has no independent field visibility yet. Normal code should use `insert`/`get`/`remove`; stronger field privacy remains a language-design item.
