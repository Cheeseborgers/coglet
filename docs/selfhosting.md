...
* dotted module/package naming, manifests, installed stdlib lookup, and a stable compilation-unit/library model
...
At this stage, the remaining path toward self-hosting is primarily runtime/library, toolchain, and compilation-unit engineering rather than another major semantic-analysis subsystem. The host-C backend and LLVM Stage 9 optimized/debuggable native path both consume frozen CogIR; named-module visibility is private-by-default with explicit exports and single-component imports now support deterministic sibling/`-I` source discovery, while major remaining gaps include dotted package/manifests and stdlib lookup, allocation and I/O facilities, a standard-library boundary, and production-grade target/cross-toolchain policy.
